...
![Infrastructure architecture](./diagrams/infra-architecture.png)
...
![CI/CD deploy flow](./diagrams/cicd-flow.png)
...
- [`infra/lambda-env.example`](./lambda-env.example) — versioned, keys only, no values. This is
  the durable reference for which vars exist.
- [`infra/scripts/pull-lambda-env.sh`](./scripts/pull-lambda-env.sh) — fetches the *live* values
  from the Lambda function into `infra/.env.lambda` (gitignored — never committed).
- [`infra/scripts/push-lambda-env.sh`](./scripts/push-lambda-env.sh) — reads `infra/.env.lambda`,
