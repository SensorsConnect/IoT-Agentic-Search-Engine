  ...
  return (
    <div>
      <div className="flex justify-between text-xs mb-1">
        ...
        <span className="text-gray-300">{label} ({(occupancy * 100).toFixed(0)}%)</span>
      </div>
  ...
  return (
    <div className="flex items-center gap-3">
      <svg width="72" height="72" viewBox="0 0 72 72">
        ...
        />
        <text x="36" y="34" textAnchor="middle" className="fill-gray-200 text-xs font-medium">
          {minutes.toFixed(0)}
        </text>
      ...
      <div>
        ...
        <p className="text-sm text-gray-200 font-medium">~{minutes.toFixed(0)} minutes</p>
      </div>
    </div>
  )
}
...
export default function PlaceDetailSheet() {
  ...
  return (
    <div className="hidden md:block">
      ...
      <div className="absolute inset-auto top-0 right-0 bottom-0 w-[400px] z-30 animate-slide-in-right">
        <div className="h-full bg-surface/95 backdrop-blur-xl border-l border-white/10 overflow-y-auto dark-scrollbar">
          {/* Close button */}
          <button
            onClick={() => setSelectedPlaceId(null)}
            className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-gray-200 hover:bg-white/10 transition-all"
          >
          ...
          <div className="p-5 space-y-4">
            ...
            <div>
              <div className="flex items-start gap-2">
                <h2 className="text-lg font-semibold text-gray-100 flex-1">{place.name}</h2>
                {isIoT && (
              ...
              <div className="bg-white/[0.03] rounded-xl p-4 border border-white/5 space-y-4">
                ...
                {place.travel_time_min != null && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-200 font-medium">{place.travel_time_min.toFixed(0)} min</span>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="flex flex-wrap gap-2">
                {place.phone && (
                  <a
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-gray-300 hover:border-neon-cyan/30 hover:text-neon-cyan transition-all"
                  >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl bg-neon-cyan/10 border border-neon-cyan/20 text-neon-cyan font-medium text-sm hover:bg-neon-cyan/20 transition-all"
            >
